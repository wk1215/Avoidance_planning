...
public:
    LocalPathPlanner()
    {
        ...
        sub_env =  nh_plan.subscribe("/env_pub", 10, &LocalPathPlanner::EnvPointCallback, this);
    ...
    void EnvPointCallback(const mdwa_planning::Environment::ConstPtr& msg)
    {
        env.barrier = msg->barrier;
        env.dy_obs = msg->dy_obs;
        env.obs_size = msg->obs_size;
    }
...
private:
    ...
    ros::Subscriber sub_env;
    ros::Rate loop_rate{ros::Rate(50)};
    // Environment  env;
    mdwa_planning::Environment    env; //存储地形障碍物
